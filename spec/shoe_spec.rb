...
describe (Brand) do
  ...
  it('validates presence of brand name') do
    shoe = Brand.create({name: '', price: 30, formatted_price: nil})
    expect(shoe.save).to eq(false)
  end
  it('validates presence of brand shoe price') do
    shoe = Brand.create({name: 'Doc Marten', price: nil, formatted_price: nil})
    expect(shoe.save).to eq(false)
  end
  ...
  it('validates brand name length does not exceed 100 characters') do
    shoe = Brand.create({name: 't' * 101, price: 100, formatted_price: nil})
    expect(shoe.save).to eq(false)
  end

  describe('#titlecase') do
    it('ensures the name of the brand is titlecased') do
      shoe = Brand.create({name: 'dOC maRTen', price: 100, formatted_price: nil})
      expect(shoe.name).to eq('Doc Marten')
    end
  end
...
describe (Store) do
  ...
  it('validates store name length does not exceed 100 characters') do
    store = Store.create({name: 'W' * 101})
    expect(store.save).to eq(false)
  end

  describe('#titlecase') do
    it('ensures the name of the store is titlecased') do
      store = Store.create({name: 'WALLY woRLD'})
      expect(store.name).to eq('Wally World')
    end
  end
